...
public class DatabaseConnection {
    ...
    public static void displayEmployeeDetails(int employeeId) throws SQLException {
        String query = "SELECT e.id, e.name, e.age, e.salary, e.email, e.phone_number, d.department_name, " +
                "a.address_type,a.street, a.city, a.state, a.zip_code " +
                "FROM employees e " +
                "JOIN departments d ON e.department_id = d.id " +
                "JOIN address a ON e.id = a.employee_id " +
                "WHERE e.id = ?";  // Added condition to filter by employee ID

        try (Connection connection = getConnection();
             PreparedStatement stmt = connection.prepareStatement(query)) {

            // Set employee ID in the query
            stmt.setInt(1, employeeId);
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                ...
                if (rs.next()) {
                    ...
                    do {
                        ...
                        System.out.println("  Address Type: " + rs.getString("address_type"));
